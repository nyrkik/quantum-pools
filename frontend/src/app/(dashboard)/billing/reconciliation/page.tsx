...
import { CreditCard, Loader2, Check, X as XIcon, ArrowRight, Mail } from "lucide-react";
...
/**
 * Phase 1 payment reconciliation surface. Three tabs:
 *
 * - Pending checks: Payment.status='pending' rows. Click to flip to
 *   completed (= funds arrived).
 * - Needs review: ParsedPayment.match_status='proposed' rows. Each
 *   carries a candidate invoice; user accepts or dismisses.
 * - Unmatched: ParsedPayment.match_status='unmatched' rows. User can
 *   pick an invoice to manually match, or dismiss.
 */
...
export default function ReconciliationPage() {
  ...
  return (
    <PageLayout
      ...
      subtitle="Review parsed payments + match to invoices"
...
function PendingChecksTab({
  ...
  if (items.length === 0) {
    return <EmptyState message="No pending checks. Mailed-check notifications appear here until you confirm the funds arrived." />;
  }
  return (
          ...
          <CardContent className="p-4 flex items-center justify-between gap-3">
            <div className="min-w-0 flex-1">
              ...
              <div className="text-sm text-muted-foreground truncate">
                {p.customer_name || "(unknown customer)"} · {p.reference_number ? `#${p.reference_number}` : ""}
              </div>
              {p.notes ? (
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              >
                {busyId === p.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Check className="h-3.5 w-3.5 mr-1" />}
                Mark received
              </Button>
            </div>
          </CardContent>
...
function NeedsReviewTab({
  items, busyId, onAccept, onDismiss,
}: {
  ...
  if (!items) return null;
  if (items.length === 0) {
    return <EmptyState message="No matches awaiting review. Ambiguous parsed payments will appear here when the matcher finds multiple candidate invoices." />;
  }
  return (
    <div className="space-y-2">
      {items.map((pp) => (
        <Card key={pp.id} className="shadow-sm">
          <CardContent className="p-4 space-y-2">
            <div className="flex items-center gap-2 flex-wrap">
              <Badge variant="secondary" className="capitalize">{pp.processor}</Badge>
            ...
            <div className="flex items-center gap-2 justify-end">
              ...
              >
                {busyId === pp.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Check className="h-3.5 w-3.5 mr-1" />}
                Accept
              </Button>
            </div>
          </CardContent>
...
function UnmatchedTab({
  items, busyId, onManualMatch, onDismiss,
}: {
  items: ParsedRow[] | null;
  busyId: string | null;
  ...
  if (!items) return null;
  if (items.length === 0) {
    return <EmptyState message="No unmatched payments. Parsed payments with no candidate invoice appear here." />;
  }
  return (
    <div className="space-y-2">
      {items.map((pp) => (
          ...
          <CardContent className="p-4 space-y-2">
            ...
            <div className="flex items-center gap-2 justify-end">
              ...
              <Input
                ...
                onChange={(e) => setInvoiceInputs({ ...invoiceInputs, [pp.id]: e.target.value })}
              ...
              >
                {busyId === pp.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <ArrowRight className="h-3.5 w-3.5 mr-1" />}
                Match
              </Button>
              <Button
                variant="ghost"
