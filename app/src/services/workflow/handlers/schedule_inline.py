"""schedule_inline — coordinator-driven post-creation handler.

Like `assign_inline` but adds a date picker. Default date = tomorrow
9am local (soft default; the user can change it).
"""
...
def _tomorrow_9am_utc() -> datetime:
    """Tomorrow at 9am in server-local time, coerced to UTC. Frontend
    can shift to the user's timezone before display."""
    tomorrow = (datetime.now(timezone.utc) + timedelta(days=1)).date()
    return datetime.combine(tomorrow, time(9, 0), tzinfo=timezone.utc)


@register
class ScheduleInlineHandler:
    name = "schedule_inline"
    entity_types = ("job",)
        ...
        *,
        ...
        return NextStep(
            ...
            initial={
                ...
                "default_date": _tomorrow_9am_utc().isoformat(),
