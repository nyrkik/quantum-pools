...
| `thread.restored_from_historical` | user_action | thread_id | User promoted a pre-cutover `is_historical=True` thread back into the active inbox. Payload: `{}`. Ingested by `app/scripts/import_historical_gmail.py`; discovered via customer detail page or All Mail, then restored when judged worth active tracking. |
...
| `thread.auto_handled` | agent_action | thread_id, customer_id? | Classifier auto-closed an inbound without sending a reply (no_response / thank_you / general-no-draft / spam / auto_reply). Stamps `agent_threads.auto_handled_at` and surfaces the thread in the AI Review folder. Payload: `{category, classifier_confidence}`. Aggregating `COUNT(*) GROUP BY category` shows where the classifier is pulling its weight vs. where it might be over-handling. |
...
| `voice.dictated` | user_action | — | Browser-native dictation produced a transcript. Payload: `{surface, duration_ms, char_count, language}` where `surface` ∈ `{deepblue_chat, email_reply, email_compose, job_notes, visit_notes, other}`. **No transcript content** — the event is privacy-by-design metadata only (content lives only in the field the user dictated into). Use to measure adoption (`GROUP BY surface`) and attrition (ratio of dictation starts with `char_count=0` to those with content). See `docs/voice-integration-plan.md`. |
| `email.body_normalized` | system_action | thread_id, agent_message_id | Inbound body needed post-parse repair — ingest normalization fired. Payload: `{provider, from_email_domain, mime_unwrapped?, qp_decoded?, html_stripped_from_text?, charset_fallback_used?, mojibake_repaired?, zero_width_stripped?}`. Each flag key is present only when that transform fired (so consumers can filter by key presence). `charset_fallback_used` = declared charset was wrong, `charset-normalizer` picked something else. `mojibake_repaired` = `ftfy.fix_text` changed the text (UTF-8 mis-decoded as Latin-1, etc.). `zero_width_stripped` = U+200B/C/D or U+FEFF removed. Emitted alongside `agent_message.received` for the same message. Use to catch new provider quirks before users report them: `SELECT from_email_domain, COUNT(*) FROM platform_events WHERE event_type = 'email.body_normalized' GROUP BY 1 ORDER BY 2 DESC`. |
...
| `handler.applied` | user_action | `entity_id`, `entity_type`, `assignee_user_id?` | Phase 4 post-creation handler completed. Payload: `{handler, input?}`. `handler` is the registry key (`assign_inline` / `schedule_inline` / `hold_for_dispatch`). `input` holds non-PII user choices (e.g., `{due_date}` for schedule_inline, `{unassigned_count}` for hold_for_dispatch); any user-id-as-value lives in `assignee_user_id` on entity_refs, never in payload. Legacy rows emitted before 2026-04-21 may contain `handler: "unassigned_pool"` with `pool_count` — renamed to avoid conflict with pool-service domain vocabulary. |
