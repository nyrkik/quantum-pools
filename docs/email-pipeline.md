...
`_persist_inbound_attachments()` walks the MIME tree and writes parts to `message_attachments`. Inline parts (`Content-Disposition: inline` + `Content-ID` header) are persisted with `is_inline=true` and the bare `content_id`; the thread API rewrites `<img src="cid:...">` body refs to attachment URLs at read time so they render in the iframe at intended size. `TRANSACTIONAL_NOTIFICATION_SENDERS` (Stripe payouts, Amex transfer, AppFolio digests) and OOO subject prefixes ("Automatic reply:", "Out of Office:") force `no_response` classification before the pending-vs-handled dispatch; `_FOLLOWUP_PROMISE_RE` overrides `no_response` back to pending when a customer commits to follow up, and stamps `agent_threads.awaiting_reply_until` for the promise-tracker widget.
