...
- **Inbox information architecture (2026-04-14, updated 2026-04-23)**: Email integrations + signature config live at `/inbox/integrations` (moved from `/settings/email`; 308 redirect in `next.config.ts`). Signature page has two stacked cards: "Your email signature" (per-user, everyone sees it — sign-off + personal info) and admin-only "Email Signature" (toggles + shared org footer). Rules at `/inbox/rules`. Compose button at top of the inbox folder sidebar (`InboxFolderSidebar`), NOT in `PageLayout` header or app sidebar. Inbox filter chips: Clients (customer-matched threads), Handled (archive view), plus ops chips (Auto-Handled / Stale, owner+admin only). Failed is not a chip — stuck outbound lives in the **Outbox** system folder (system_key="outbox") alongside Inbox/Sent/Spam/All Mail/Historical. Thread row badges in fixed order: Sender tag → Category → Status → Stale.
