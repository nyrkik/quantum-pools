...
- **Inbox information architecture (2026-04-14, updated 2026-04-25)**: Email integrations + signature config live at `/inbox/integrations` (moved from `/settings/email`; 308 redirect in `next.config.ts`). Signature is ONE card with three stacked sections: Preview (live) → Your personal info (per-user, everyone) → Organization settings (admin-only). Rules at `/inbox/rules`. Compose button at top of the inbox folder sidebar (`InboxFolderSidebar`), NOT in `PageLayout` header or app sidebar. Inbox filter row (post-2026-04-25 rework): **Mine** toggle · segmented status control (`All | Pending | Handled` — same options for every role) · **Clients** toggle · gear icon → settings sheet · search. The 2026-04-24 admin-only `Auto` segment was removed — auto-handled threads now surface in the **AI Review** system folder (`system_key="ai_review"`, owner+admin only via `inbox.manage`, amber sidebar badge) and in the regular Handled segment for every role with the row-level "AI" pill. "Group threads by client" is a view-mode toggle inside the settings sheet, not a filter chip. Stale mail surfaces as a dismissible banner above the thread list when `stats.stale_pending > 0` (owner+admin only) — clicking Show switches to a stale-only view with a Clear CTA. Failed is not a chip — stuck outbound lives in the **Outbox** system folder (system_key="outbox") alongside Inbox/Sent/Spam/AI Review/All Mail/Historical. Red badge color is reserved for Outbox only — every other folder + the main sidebar Inbox unread count use primary/navy (AI Review uses amber). Thread row badges in fixed order: Sender tag → Category → Status → AI → Stale (both amber, not red).
