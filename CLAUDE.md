...
- **Backend** (`app/`): `app.py` factory, `worker.py` (APScheduler), `src/{api,core,middleware,models,schemas,services,seeds,utils}`. Services are grouped by domain: `agents/`, `deepblue/`, `inspection/`, `gmail/`, `parts/`, `payments/`, plus top-level `billing_service.py`, `thread_action_service.py`, `thread_ai_service.py`, `estimate_workflow_service.py`, `stripe_service.py`, `customer_portal_service.py`.
...
- [~] Phase 3c: Invoicing — webhook PARTIAL; email/PDF/Stripe Checkout/pay page/non-client invoices/AutoPay DONE. **2026-04-28**: A/R aging report (backend + UI tab on Invoices, sortable, CSV export); Reconciliation IA folded into Invoices page as a tab (sidebar entry removed; `/billing/reconciliation` redirects); Payment reminders / Phase 4 dunning shipped manual-only — `BillingService.{preview_dunning_sequence,run_dunning_sequence,send_dunning_email}` + 4 escalating templates (initial / 3-day / service-at-risk / final notice) + admin endpoint `POST /v1/billing/dunning/run?dry_run=true|false` w/ optional `invoice_ids[]` body for selective send + UI tab w/ checkboxes & sortable columns; PSS-imported invoices excluded from dunning by design (manual-review rule). Auto-scheduler in `agent_poller` is intentionally commented out — backlog needs human review before auto-fire. **2026-04-29 (Phase 8)**: Late fees + service holds shipped manual-only. Org-level policy on Organization (`late_fee_enabled/type/amount/grace_days/minimum`); per-customer override `late_fee_override_enabled` (null=inherit, true/false=force). `BillingService.{preview_late_fees,run_late_fees}` + idempotent line-item application (`description LIKE 'Late fee%'` AND `service_id IS NULL` — no schema change to InvoiceLineItem). T+14 dunning email mentions impending fee when org has policy enabled. Settings → Billing "Late Fee Policy" card (owner-only); Invoices → 6th "Late fees" tab mirrors dunning preview UX. New `property_holds` table (`PropertyHoldService`) — recurring billing skips a customer when ALL of their active properties have a hold covering the billing period start (single-property residential skips cleanly; multi-property partial-hold still bills). Customer detail "Service Holds" tile with date-range + reason; Add/Delete via dialog. PSS-imported invoices excluded from late fees by design (mirrors dunning rule). Auto-scheduler intentionally NOT wired — manual-only same posture as dunning.
- [~] Phase 3d: Core Pool Ops — dosing engine + checklists PARTIAL; guided workflows NOT STARTED. Phase 3d.2 (LSI gauge + DosingCards on visit-readings) shipped 2026-04-27 — `/v1/chemistry/water-features/{bow_id}/{lsi,dosing}` endpoints, deterministic dosing with AgentLearningService correction capture, 3-band LSI gauge.
...
- [x] Promise tracker — shipped 2026-04-27. `agent_threads.awaiting_reply_until` set on inbound followup-promise (`_is_followup_promise` regex), cleared on subsequent inbound. `GET /v1/inbox/awaiting-reply` + `PUT /v1/admin/agent-threads/{id}/awaiting-reply` for snooze/clear; `<AwaitingReplyWidget>` on dashboard for owner+admin. Closes the gap left by the outbound-invisible rule.
- [x] Inbox badging audit — shipped 2026-04-27. Removed cross-folder dashboard pills (pending/stale); badges live in their folders only (Gmail/Outlook convention). `TRANSACTIONAL_NOTIFICATION_SENDERS` + OOO-subject-prefix overrides force `no_response` for known FYI-only senders. JSONB `none_as_null=True` on `agent_threads.visibility_role_slugs` (Python `None` → SQL NULL, otherwise rows became invisible to every role). Owner/admin/manager presets gain `inspection.*` + `inbox.see_all_mail` (the seed script is the source of truth — migrations alone weren't enough).
- [x] Inline image rendering — shipped 2026-04-27. `message_attachments` gains `content_id` + `is_inline`; CID-referenced inline parts are persisted with `is_inline=true` and excluded from the attachments grid. Body-html `<img src="cid:...">` rewritten to `/api/v1/attachments/{id}/file` (or `Organization.logo_url` for outbound `qp-signature-logo`). Iframe sandbox gains `allow-same-origin` so cookie-authenticated img loads work.
- [~] Voice (FB-29) Phase 1 — shipped 2026-04-27. `<VoiceDictationButton>` (Web Speech API, feature-detected) wired into DeepBlue chat, email compose, job notes, and inbox thread reply. Emits `voice.dictated` events `{surface, duration_ms, char_count, language}` — no transcript content. Phase 2 (Groq Whisper) + Phase 3 (streaming + TTS) deferred until adoption signal.
...
- [~] Phase 4: Customer Portal — **V1 shipped 2026-04-28**. Magic-link sign-in (15-min single-use tokens via `customer_magic_links`) + persistent session cookie (`customer_portal_sessions`, 30-day sliding expiry, idle-refresh threshold 5 min). `CustomerPortalService` covers request/consume/get/revoke/sweep_expired. Public surface at `/v1/portal/{request-link, consume, me, invoices, payments, autopay, logout}` (cookie-authed, distinct from staff JWT; `qp_portal_session` cookie HttpOnly). Frontend at `/portal/login`, `/portal/login/[token]`, `/portal` (3 tabs: Billing / Payment methods / History; org-branded). Email enumeration protection: `request-link` ALWAYS returns `{ok:true}`. Multi-recipient send loop (one email per recipient, partial-failure tolerance, dedup case-insensitive). Add-payment-method in-portal deferred to Phase 4 polish. `sweep_expired` not yet wired into agent_poller (housekeeping; rows don't grow problematic at current scale).
...
| `docs/billing-may-launch-plan.md` | Per-phase plan for May 2026 billing go-live: Stripe Connect Standard + ACH + magic-link customer portal + dunning + A/R aging (must-have), then consolidated multi-property billing + statements + late fees + holds + partial payments + credits. Built from 2026-04-28 competitive + best-practice research. **Remove when billing is fully live and stable for 30+ days** (same trigger as `billing-dormant.md`). |
