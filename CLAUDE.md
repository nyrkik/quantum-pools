...
| `docs/ai-platform-phase-5.md` | Phase 5 refinement spec — unify existing agents (`estimate_generator`, `email_drafter`, `customer_matcher`) into the proposal system. New creators + renderers for `email_reply` and `customer_match_suggestion`, pilot sequencing, flag-gated email-drafter rollout, historical `draft_response` port script, R1 audit for all `draft_response` references, 15-item DoD. **Remove when Phase 5 shipped + archived.** |
