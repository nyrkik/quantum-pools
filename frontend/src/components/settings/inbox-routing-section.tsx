...
export function InboxRoutingSection({ editMode }: { editMode: boolean }) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setAdding(false);
      setNewRule({ address_pattern: "", match_type: "exact", category: "", required_permission: "", priority: 0 });
      loadRules();
    } catch {
    ...
    <Card className="shadow-sm">
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-base">Inbox Routing</CardTitle>
            <CardDescription>
              Route incoming emails to the right team members based on the
              receiving address. Rules match by priority (lower = first).
            </CardDescription>
          </div>
          {editMode && (
          ...
          <div className="space-y-4">
            ...
            {rules.length === 0 && !adding ? (
              <p className="text-sm text-muted-foreground text-center py-6">
                No routing rules configured. All emails will be visible to
                everyone with inbox access.
              </p>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-100 dark:bg-slate-800">
                    <TableHead className="text-xs font-medium uppercase tracking-wide">Address Pattern</TableHead>
                    <TableHead className="text-xs font-medium uppercase tracking-wide">Match</TableHead>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  ...
                                  <strong>{rule.address_pattern}</strong>. Existing
                                  threads will keep their current visibility.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
