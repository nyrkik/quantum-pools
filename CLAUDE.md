...
- **Inbox information architecture (2026-04-14, updated 2026-04-24)**: Email integrations + signature config live at `/inbox/integrations` (moved from `/settings/email`; 308 redirect in `next.config.ts`). Signature is ONE card with three stacked sections: Preview (live) → Your personal info (per-user, everyone) → Organization settings (admin-only). Rules at `/inbox/rules`. Compose button at top of the inbox folder sidebar (`InboxFolderSidebar`), NOT in `PageLayout` header or app sidebar. Inbox filter row (post-2026-04-24 rework): **Mine** toggle · segmented status control (`All | Pending | Handled | Auto` — Auto-segment admin-only) · **Clients** toggle · gear icon → settings sheet · search. "Group threads by client" is a view-mode toggle inside the settings sheet, not a filter chip. Stale mail surfaces as a dismissible banner above the thread list when `stats.stale_pending > 0` (owner+admin only) — clicking Show switches to a stale-only view with a Clear CTA. Failed is not a chip — stuck outbound lives in the **Outbox** system folder (system_key="outbox") alongside Inbox/Sent/Spam/All Mail/Historical. Red badge color is reserved for Outbox only — every other folder + the main sidebar Inbox unread count use primary/navy. Thread row badges in fixed order: Sender tag → Category → Status → Stale (amber, not red).
...
- **Phase 5 AI Platform migration (shipped 2026-04-24)**: `estimate_generator`, `email_drafter`, and `customer_matcher` all stage `agent_proposals` rows (entity types `estimate`, `email_reply`, `customer_match_suggestion`). Canonical creators under `app/src/services/proposals/creators/`. Low-confidence matches go through owner+admin review at `/inbox/matches`. Inbox reading pane renders `<ProposalCard>` exclusively — no legacy draft drawer. `classifier.get_correction_history` + `thread_action_service.approve_thread` + `handle_sms_reply` + `POST /agent-threads/{id}/approve` + `save-draft` + `/agent-messages/{id}/approve|reject|dismiss` all deleted; every draft flows through `ProposalService.accept`. Phase 5b (same day) dropped `agent_messages.draft_response`; R7 audit enforcer blocks any `.draft_response` attribute access in `app/src/` to prevent regressions. `final_response` still written by the email_reply creator (denormalization cleanup is a separate future project).
...
| `docs/auto-handled-folder-plan.md` | Thread-status cleanup + AI Review folder. Fixes the `thread.status="ignored"` conflation that hides auto-handled inbound (e.g. Workspace notifications) from the default Inbox. Adds `auto_handled_at` column, rewrites `update_thread_status` derivation, replaces admin-only Auto segment with a sidebar folder + amber badge visible to owner/admin, surfaces auto-handled in the regular Handled segment for non-admins. Includes query audit, backfill migration, DoD, risks. **Remove when shipped + archived.** |
