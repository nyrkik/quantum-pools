...
- **Inbox information architecture (2026-04-14, updated 2026-04-23)**: Email integrations + signature config live at `/inbox/integrations` (moved from `/settings/email`; 308 redirect in `next.config.ts`). Signature is ONE card with three stacked sections: Preview (live) → Your personal info (per-user, everyone) → Organization settings (admin-only). Rules at `/inbox/rules`. Compose button at top of the inbox folder sidebar (`InboxFolderSidebar`), NOT in `PageLayout` header or app sidebar. Inbox filter chips: Clients (customer-matched threads), Handled (archive view), plus ops chips (Auto-Handled / Stale, owner+admin only). Failed is not a chip — stuck outbound lives in the **Outbox** system folder (system_key="outbox") alongside Inbox/Sent/Spam/All Mail/Historical. Red badge color is reserved for Outbox only — every other folder + the main sidebar Inbox unread count use primary/navy. Thread row badges in fixed order: Sender tag → Category → Status → Stale (amber, not red).
- **Email signature pipeline (2026-04-23)**: `src/services/email_signature.compose_signature` is the single source of truth — called from `EmailService.send_agent_reply` and the `/v1/auth/me/email-signature/preview` endpoint so the preview is literally what ships. Composition order (top→bottom): optional sign-off → auto-prepended {first_name, org_name} (if admin enables) → user personal text → org shared footer → inline CID logo (clickable when `organizations.website_url` is set). Line-level dedup prevents the org name rendering twice. HTML body gets autolinked emails (`mailto:`), phones (`tel:`), bare domains (`https://`). Admin can turn off per-user customization via `organizations.allow_per_user_signature` for absolute brand consistency. Legacy-identity bleed protection: `send_agent_reply` overrides `from_address` to `agent_from_email` if the address isn't on the org's primary domain — stops converted-personal-Gmail addresses from leaking outbound.
- **Classifier first-contact guardrail (2026-04-22)**: In `orchestrator.py`, if an inbound message classified as `no_response`/`thank_you` comes from a sender with no prior `agent_messages` rows for the org AND no matched customer, status is forced to `pending` regardless of classifier verdict. Prevents first-contact unknowns (test emails, new prospects) from being silently auto-ignored. Shows up in Inbox normally; AgentLearningService can train the classifier to be more accurate over time without losing user visibility.
- **Historical threads (2026-04-22)**: `agent_threads.is_historical=True` marks pre-cutover mail imported via `app/scripts/import_historical_gmail.py`. Excluded from triage — `list_threads` default query and `get_thread_stats` both skip historical. Included when `customer_id` scopes the query (customer detail page) or `folder_key == "all"` (archive search). Thread detail action bar shows an `ArchiveRestore` icon only when `is_historical=True`; clicking calls `/v1/admin/agent-threads/{id}/restore-to-inbox` which flips the flag and emits `thread.restored_from_historical`. Ingest is idempotent on re-run via `thread_key = "hist|<gmail_thread_id>"`. Script runs under a separate GCP project (`quantumpools-oauth`) with Desktop OAuth token at `~/.config/qp-sapphire-cleanup/token.json`.
- **Per-thread rule entry point (2026-04-22)**: `Wand2` icon in the thread detail action bar navigates to `/inbox/rules?thread_id=<id>`. Rules page reads the param, fetches the thread via `/v1/admin/agent-threads/{id}`, and shows a purple banner + inline "Add sender" buttons on every rule whose conditions include `sender_email equals` or `sender_domain equals` (calls existing `/v1/inbox-rules/{id}/append-sender`). The top "Add rule" button prefills a new draft from the thread (sender + folder/tag/category) when `thread_id` is present. Dismiss X clears the param via `router.replace`. One surface for both "add to existing rule" and "create new rule" — no in-thread modal.
...
- **Phase 5 AI Platform migration (2026-04-23)**: Four agent paths now stage `agent_proposals` rows instead of writing directly to DB. Entity types added: `estimate` (re-wired Step 1), `email_reply` (Step 2), `customer_match_suggestion` (Step 3). Flag-gated by env var `EMAIL_DRAFTER_USE_PROPOSALS` in `app/.env` (currently `true` on Sapphire). When flag is on: orchestrator stages `email_reply` proposals instead of writing `AgentMessage.draft_response`; fuzzy customer matches that fail Claude QC stage `customer_match_suggestion` proposals for human review at `/inbox/matches` (owner+admin). Canonical creators under `app/src/services/proposals/creators/`. ProposalCard's `EDITABLE_TYPES` registry controls which types support inline edit; estimate + email_reply are editable, customer_match_suggestion is read-only. **Step 4 (2026-04-23): historical port complete** — 150 Sapphire `draft_response` rows ported via `app/scripts/port_draft_response_to_proposals.py` (idempotent, parity-asserted). `classifier.get_correction_history` deleted; `classify_and_draft` injects corrections via `build_lessons_prompt(AGENT_EMAIL_DRAFTER)` + past-conversation context from a fresh outbound-body query. `thread_action_service.approve_thread` + `POST /agent-threads/{id}/approve` + `save-draft` endpoint + legacy draft blocks in `thread-detail-sheet.tsx` / `action-detail-content.tsx` all deleted — every draft now flows through `ProposalService.accept`.
- **Navigation provider (2026-04-23)**: `lib/nav-history.tsx` (`NavHistoryProvider` wraps the `(dashboard)` layout) tracks pathname + searchParams changes in a session-scoped stack. `<BackButton fallback="..." />` component in `components/ui/back-button.tsx` owns Back-button resolution order: `?from=` query param → `NavHistoryProvider` prev → `router.back()` → `fallback` prop. All 8 in-app Back buttons use it. Inbox thread selection is URL-driven (`/inbox?thread=<id>` via `router.replace`) so navigating away and back restores the open thread sheet. Any new page that wants "Back returns me to where I came from" gets it for free — just use `<BackButton fallback="/some/route" />`.
- **Case detail pending proposals section (2026-04-23)**: `/cases/[id]` endpoint returns `pending_proposals[]` — staged proposals sourced from case-linked threads OR carrying `case_id` in payload. Case detail page renders `<ProposalCard>` for each at the top. Accept/Edit/Save commits materialize the outcome entity atomically (Invoice for estimate, outbound AgentMessage for email_reply, thread.matched_customer_id for customer_match_suggestion) and auto-selects the new timeline entry in the right detail panel.
...
- [~] Phase 5 (AI Platform): Steps 1-4 DONE — estimate_generator pilot, email_drafter migration (flag-gated via `EMAIL_DRAFTER_USE_PROPOSALS`, live on Sapphire 2026-04-23), customer_matcher low-confidence review queue at `/inbox/matches`, historical port of 150 `draft_response` rows + legacy-approve-path removal (2026-04-23). Step 5 (audit enforcer + closeout — remaining `draft_response` reads across 7 files) REMAINING. Spec: `docs/ai-platform-phase-5.md`.
...
| `docs/ai-platform-phase-2.md` | Phase 2 implementation spec — `agent_proposals` table + `ProposalService` + entity-type registry. Structural enforcement of DNA rules "every agent learns" and "AI never commits to customer" — every AI suggestion becomes a first-class proposal record. **Remove when Phase 2 shipped + archived.** |
...
| `docs/ai-platform-phase-4.md` | Phase 4 refinement spec — post-creation handlers. `org_workflow_config` table, `WorkflowHandler` protocol, 3 shipping handlers (assign_inline/unassigned_pool/schedule_inline), accept-response `next_step` extension, plain-language settings surface, 11-item DoD. **Remove when Phase 4 shipped + archived.** |
| `docs/ai-platform-phase-5.md` | Phase 5 refinement spec — unify existing agents (`estimate_generator`, `email_drafter`, `customer_matcher`) into the proposal system. **Steps 1-3 SHIPPED 2026-04-23; Steps 4 (historical port) + 5 (audit + closeout) REMAINING.** New creators + renderers for `email_reply` and `customer_match_suggestion`, inbox reading-pane swap, `/inbox/matches` review queue, flag-gated email-drafter rollout via `EMAIL_DRAFTER_USE_PROPOSALS`. **Remove when Phase 5 fully shipped + archived.** |
