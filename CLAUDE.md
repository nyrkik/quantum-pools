...
- **Phase 5 AI Platform migration (shipped 2026-04-24)**: `estimate_generator`, `email_drafter`, and `customer_matcher` all stage `agent_proposals` rows (entity types `estimate`, `email_reply`, `customer_match_suggestion`). Canonical creators under `app/src/services/proposals/creators/`. Low-confidence matches go through owner+admin review at `/inbox/matches`. Inbox reading pane renders `<ProposalCard>` exclusively — no legacy draft drawer. `AgentMessage.draft_response` is dead runtime state (R7 audit enforces zero attribute access in `app/src/`); column stays pending Phase 5b schema drop. `classifier.get_correction_history` + `thread_action_service.approve_thread` + `handle_sms_reply` + `POST /agent-threads/{id}/approve` + `save-draft` + `/agent-messages/{id}/approve|reject|dismiss` all deleted; every draft flows through `ProposalService.accept`. Historical port script at `app/scripts/port_draft_response_to_proposals.py` (idempotent, parity-asserted) already ran on Sapphire — 150 rows → 150 proposals + 144 corrections.
...
- [x] Phase 5 (AI Platform) — shipped 2026-04-24. Three agents unified into `agent_proposals`, historical port complete, legacy paths + `EMAIL_DRAFTER_USE_PROPOSALS` flag retired, R7 enforcer blocks regressions. Column drop deferred to Phase 5b. Spec archived.
