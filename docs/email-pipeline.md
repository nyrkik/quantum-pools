...
**Auto-handled review loop (three-state banner as of 2026-04-14):** When the AI hides an email from the Inbox without sending a reply (moves to Spam, custom folder via tag, or just marks ignored), the thread is flagged `is_auto_handled`. The reading pane's banner is driven by `GET /v1/inbox-rules/match-preview?thread_id=…` on mount, which inspects existing active rules and returns one of three states:
- **covered** — an existing rule already catches this sender. Banner just acknowledges; no decision to make.
- **promotable** — exactly one rule has the matching action + sender-based condition and does NOT yet include this sender. Banner's primary button is "Add to rule" which calls `POST /v1/inbox-rules/{rule_id}/append-sender` and atomically appends the sender to that rule's `value` list (promoting scalar→array if needed). Keeps rule sets from bloating with one-off new rules. Also fires an `AgentLearningService` acceptance signal.
- **unclear** — no matching rule, or 2+ rules match ambiguously. Falls back to the classic Yes / Create-rule / No banner so the user picks explicitly.

All three paths ultimately feed `AgentLearningService` (`agent_type=email_classifier`). The "No, move to Inbox" path also restores the thread to Inbox + marks pending. Filter button "Auto-Handled" + Inbox folder count chip surface the backlog. `is_auto_handled` is derived in `thread_presenter` from `(last_direction=inbound AND status IN (ignored, handled) AND NOT has_pending AND auto_handled_feedback_at IS NULL)` — the ack timestamp (set on every Yes/No click) permanently silences the banner for a thread so it doesn't reappear on each open.
...
**Phase 5 email_drafter migration (2026-04-23)**: when env var `EMAIL_DRAFTER_USE_PROPOSALS=true` (currently on for Sapphire), the classifier's `draft_response` output is NOT written to `AgentMessage.draft_response`. Instead the orchestrator stages an `email_reply` proposal via `ProposalService.stage(entity_type="email_reply", ...)` with source_id = the inbound `AgentMessage.id`. The proposal carries the draft in its `proposed_payload.body`. The inbox reading pane fetches the proposal by id and renders `<ProposalCard>` in place of the legacy draft drawer. Accept → `proposals/creators/email_reply.py` delegates the send to `EmailService.send_agent_reply` (canonical outbound path), marks inbound sent, creates outbound `AgentMessage`, and recomputes thread status — all atomic inside `ProposalService.accept`'s transaction. Edit → `agent_corrections` row with the diff, feeding the learning loop. Flag off = legacy `draft_response` flow unchanged.
...
**Phase 5 customer_matcher extension (2026-04-23)**: high-confidence trusted-method matches (`email`, `contact_email`, `previous_match`, `sender_name`) still auto-apply unchanged — deterministic join logic, not AI commitment. When a fuzzy match fails Claude QC verification, the dropped candidate is appended to an `unverified_sink` the orchestrator passes in. When `EMAIL_DRAFTER_USE_PROPOSALS=true`, the orchestrator stages a `customer_match_suggestion` proposal for each unverified candidate (source_type="thread", source_id=thread.id, payload carries `candidate_customer_id`, `reason`, `confidence`). Owner+admin review at `/inbox/matches`; Accept applies `thread.matched_customer_id`; Reject preserves unmatched + records correction.
