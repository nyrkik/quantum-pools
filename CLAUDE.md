...
- **Existing coverage** (58 tests, see `tests/`): webhook signature validation (5), cross-org isolation on every fetch-by-id pattern (5), send-failure recovery FB-24 class (3), Failed filter latest-only semantic (4), Redis graceful degradation + auto-recovery (5), inbox-rules service (18), case-link service, no-auto-send guard (4), Gmail spam sync + retention (5).
